...
### NOTE: document is broken down into two parts. Results are reported first, and assumption testing (Shapiro-Wilk for normality, Levene's for homogeneity of variance) is reported second.



source("00_setup.R")
source("05_alpha_diversity.R")
#library(vegan)
#library(microbiome)
...
merge_samdat %>% 
  ...
  group_by(AVA) %>% 
  summarise(mean=mean(Growing.Degree.Days),
            sd=sd(Growing.Degree.Days),min=min(Growing.Degree.Days), max=max(Growing.Degree.Days)) %>% 
...
merge_samdat %>% 
  ...
  group_by(AVA) %>% 
  summarise(mean=mean(Precipitation.growing),
            sd=sd(Precipitation.growing), min=min(Precipitation.growing), max=max(Precipitation.growing)) %>% 
  kable(caption="  \n**2016 means, SDs, min, max (by AVA)**")
...
# t test 
t.test(x=merge_samdat %>% filter(Year ==2016) %>% select(Must_pH)%>% unlist %>% as.numeric(), 
       y=merge_samdat %>% filter(Year ==2017) %>% filter(Vineyard != "AV2" ) %>% filter(Vineyard != "OR2")%>%
         select(Must_pH) %>% unlist %>% as.numeric(), 
       paired = TRUE)%>% tidy %>%
...
merge_samdat %>% 
  ...
  group_by(AVA) %>% 
  summarise(mean=mean(Must_pH),
            sd=sd(Must_pH), min=min(Must_pH), max=max(Must_pH)) %>% 
  kable(caption="  \n**2016 means, SDs, min, max (by AVA)**")
...
t.test(x=merge_samdat %>% 
       ...
       paired = TRUE)%>% tidy %>%
  kable(caption="  \n**Paired t-test of TA values between vintages**")
...
aov(formula = Must_TA ~AVA,
    data = merge_samdat %>% 
      filter(Year==2016)) %>% 
  tidy %>% kable(caption = "  \n**One-way ANOVA of TA values (2016 vintage)**")
...
aov(formula = Must_TA~AVA,
    data = merge_samdat %>% 
      filter(Year==2017)) %>% 
  tidy %>% kable(caption = "  \n**One-way ANOVA of TA values (2017 vintage)**")
...
merge_samdat %>% 
  ...
  group_by(AVA) %>% 
  summarise(mean=mean(Must_TSS),
            sd=sd(Must_TSS), min=min(Must_TSS), max=max(Must_TSS)) %>%
  kable(caption="  \n**2016 means, SDs, min, max (by AVA)**")
...
aov(formula = Must_TSS %>% log ~AVA,
    data = merge_samdat %>% 
      filter(Year==2017)) %>% 
  tidy %>% kable(caption = "  \n**One-way ANOVA of log transformed TSS values (2017 vintage)**")
...
t.test(x=fungt_alpha_test_df %>% 
       ...
       y=fungt_alpha_test_df %>% 
         ...
         select(Richness) %>% unlist %>% log, 
       paired = TRUE)%>% tidy %>%
  kable(caption="  \n**Paired t-test of log transformed richness values between vintages**")
...
aov(formula = `Inverse Simpson`~AVA,
    data = fungt_alpha_test_df %>% 
      filter(Year==2017)) %>% 
  TukeyHSD()%>% tidy %>% kable(caption="  \n**Tukey's HSD of inverse Simpson values (2017 vintage)**")
...
select(Must_TSS)%>% unlist %>% log) %>%tidy %>% 
  kable(caption="  \n**Shapiro Wilk of log transformed TSS values (2017 vintage)**")
...
select(AVA) %>% unlist)%>% 
  tidy %>% 
  kable(caption="  \n**Levene's test on log transformed TSS values (2017 vintage)**")
...
filter(Year==2016) %>% 
select(AVA) %>% unlist)%>% tidy %>%
  kable(caption="  \n**Levene of log inverse Simpson values (2016 vintage)**")
