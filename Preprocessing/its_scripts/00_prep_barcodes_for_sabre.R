...
barcodes <- read.csv("inputs/ITS_primer_barcodes.csv", stringsAsFactors = F)
barcodes <- barcodes[1:(nrow(barcodes) -1) , c("sequence_name", "BlendID")]
...
write.table(barcodes, "inputs/its_primer_barcodes_sabre.tsv", row.names = F, quote = F, sep = "\t", col.names = F)
